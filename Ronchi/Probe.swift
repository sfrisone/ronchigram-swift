...
public class Probe: NSObject {
    
    //setting default values for some things and initializing the other variables
    public var rows:Int
    public var columns:Int
    public var waveSize:Int = 256
    public var realSize:Float = 74.0
    public var lambda:Float = 0.0197
    public var apertureSize:Float = 10.0
    public var waveFunction:Matrix
    public var aperture:Matrix
    public var setAberrations = [Aberratio]()
    //public var fft:FFTController
    ...
    override init() {
        ...
        //fft = FFTController(aperture)
        //obj-c code: fftController = [[SAFFTController alloc] initWithInput:aperture Output:wavefunction];
    }
    
    // initialize so that Probe(Int, Int) gives the probe a number of rows and columns
    public init (_ numberRows:Int, _ numberColumns:Int) {
        ...
        //self.fft = FFTController(self.aperture)
    }
    
    // initialize so that Probe(Int, Int, Array<Aberratio>) gives the probe a number of rows
    // and columns, and a set of aberrations
    // this is actually useful for initializing in the ViewController, unlike probeWithAberration
    public init (_ numberRows:Int, _ numberColumns:Int, _ aberrations:Array<Aberratio>) {
        ...
        self.setAberrations = aberrations
    }
    
    //applying aberrations to a probe
    public func probeWithAberration (_ newAberrations:Array<Aberratio>, _ realSize:Float, _ apertureSize:Float, _ wavelength:Float, _ pixels:NSSize) -> (Probe) {
    ...
    //in-place aberration setter
    public func probeWithAberrations (_ newAberrations:Array<Aberratio>) {
        self.setAberrations = newAberrations
    }
            ...
            for j in jStart..<jMid + numXpix {
                ...
                if kMag <= maxK2 {
                    ...
                    for aberration in self.setAberrations {
                        ...
                        lk0mlk1 = cpow(Complex(self.lambda*k[0], -1*self.lambda*k[1]), Int(m))
                    ...
                    // sets calculated values for self.aperture
